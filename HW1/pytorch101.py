...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for t in torch.range(start,stop):
    if t%10==0:
...
def slice_assignment_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  b = torch.tensor([[0,1,2,2,2,2],[0,1,2,2,2,2],[3,4,3,4,5,5],[3,4,3,4,5,5]])
  x[:4,:6] = b
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def make_one_hot(x):
  ...
  """
  y = torch.zeros(x.shape[0],max(x)+1)
  #############################################################################
  #                    TODO: Implement this function                          #
  #############################################################################
  # Replace "pass" statement with your code
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop == False :
    ...
    z = torch.mm(x[0],y[0])
    for i in range(1,B):
      z = torch.stack([z,torch.mm(x[i],y[i])])
...
def normalize_columns(x):
  ...
  M,N = x.shape
  x_mean = x.mean(dim = 1)
  x_std = torch.sqrt(1/N*torch.sum((x - x_mean)**2,dim = 1) )
