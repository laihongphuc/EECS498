...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = torch.sqrt(dists)
  """
  x_train_flat = x_train_flat.unsqueeze(dim = 1)
  x_test_flat = x_test_flat.unsqueeze(dim = 0)
  dists = torch.sqrt(torch.sum((x_train_flat-x_test_flat)**2,dim = 2))
  """
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _,idx_test = torch.topk(dists.t(),dim = 1,largest = False,k = k)
  lable_pred = y_train[idx_test]
  
  c,d = torch.mode(lable_pred,dim = 1)
  y_pred = c
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    my_dists = compute_distances_no_loops(self.x_train, x_test)
    y_test_pred = predict_labels(my_dists, self.y_train, k=k)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_test_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_tuple = torch.chunk(x_train,num_folds,dim = 0)
  y_train_tuple = torch.chunk(y_train,num_folds,dim = 0)
  for i in range(len(x_train_tuple)):
    x_train_folds.append(x_train_tuple[i])
    y_train_folds.append(y_train_tuple[i])
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      x_train_folds_clone = x_train_folds.copy()
      y_train_folds_clone = y_train_folds.copy()
      x_val = x_train_folds_clone.pop(i)
      y_val = y_train_folds_clone.pop(i)
      my_x_train = torch.cat(x_train_folds_clone,dim = 0)
      my_y_train = torch.cat(y_train_folds_clone,dim = 0)
      classifier = KnnClassifier(my_x_train,my_y_train)
      my_acc = classifier.check_accuracy(x_val,y_val,k = k)
      my_list.append(my_acc)
    k_to_accuracies[k] = my_list
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
