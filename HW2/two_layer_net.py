...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    scores_softmax = torch.exp(scores) / torch.sum(torch.exp(scores),dim = 1).view(-1,1)
    row_idx = range(N)
    loss = - torch.sum(torch.log(scores_softmax[row_idx,y]))
    loss = loss / N
    loss += torch.sum(reg*W1*W1) + torch.sum(reg*W2*W2)
    ...
    """
    grads = {}
    y_score = torch.zeros_like(scores_softmax)
    y_score[range(N),y] = 1
    da2 = torch.log(scores_softmax)
    #dZ2 = y_score - torch.sum(torch.multiply(y_score,scores_softmax),dim = 1).view(-1,1)
    #print(torch.multiply(y_score,scores_softmax))
    dZ2 = -(y_score - scores_softmax)
    ...
    print(scores_softmax)
    """
    dW2 = torch.mm(h1.t(),dZ2)
    db2 = torch.sum(dZ2,dim = 0)
    da1 = torch.mm(dZ2,W2.t())
    d_relu_a1 = torch.zeros_like(h1)
    d_relu_a1[h1 > 0] = 1
    dZ1 = torch.multiply(d_relu_a1,da1)
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-2,1e-1,5e-1,9e-1,1e0]
  hidden_sizes = [400,450,500,550]
  regularization_strengths = [1e-5,5e-5,1e-4,5e-4]
  learning_rate_decays = [1.0,0.95,0.93]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  ...
  for lr in learning_rates:
    for h_size in hidden_sizes:
      for reg in regularization_strengths:
        ...
        net = TwoLayerNet(3 * 32 * 32, h_size, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
