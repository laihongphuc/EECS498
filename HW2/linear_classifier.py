...
def svm_loss_vectorized(W, X, y, reg):
  ...
  score = torch.mm(X,W)
  idx = range(N)
  loss_mat = score - score[idx,y].view(-1,1) +1
  loss_mat[idx,y] = 0
  loss = torch.sum(loss_mat[loss_mat>0])/N
  ...
  #############################################################################
  # Replace "pass" statement with your code
  binary = torch.zeros_like(loss_mat)
  binary[loss_mat>0] = 1
  binary[idx,y] = -torch.sum(binary,dim = 1)
  dW_t = dW.t()
  dW_t+= torch.mm(binary.t(),X)
  dW /= N
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = 100 * (torch.mean((y_train == y_train_pred).double())).item()
  val_acc = 100 * (torch.mean((y_val == y_val_pred).double())).item()
...
def softmax_loss_naive(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  my_score = torch.mm(X,W)
  my_max,_ = my_score.max(dim = 1)
  my_score = my_score - my_max.view(-1,1)
  for i in range(num_train):
    soft_max = torch.sum(torch.exp(my_score[i,:]))
    for j in range(num_class):
      if j == y[i]:
        loss -= torch.log(torch.exp(my_score[i,j]) / torch.sum(torch.exp(my_score[i,:]))) 
        #soft_max = torch.sum(torch.exp(my_score[i,:]))
        #dW += (1/soft_max) * torch.mul((X[i,:].t()).view(-1,1),torch.exp(my_score[i,:]).view(-1,1))
        #c = (1/soft_max) * torch.mul((X[i,:].t()).view(-1,1),torch.exp(my_score[i,:]).view(-1,1))
        dW[:,y[i]] -=  X[i,:]
      dW[:,j] += (1/soft_max) * torch.exp(my_score[i,j]) * X[i,:]
  loss += reg * torch.sum(W*W)
  dW += 2*W*reg
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  num_class = W.shape[1]
  y_label = torch.zeros(num_train,num_class,device  = y.device)
  y_label[range(num_train),y] = 1
  ...
  # Replace "pass" statement with your code
  my_score = torch.mm(X,W)
  #my_score = my_score - torch.max(my_score,dim = 1)
  my_max,_ =  my_score.max(dim = 1)
  my_score -= my_max.view(-1,1)
  row_idx = range(num_train)
  soft_max = torch.exp(my_score) / torch.sum(torch.exp(my_score),dim = 1).view(-1,1)
  loss = -torch.sum(torch.log(torch.exp(my_score[row_idx,y])/torch.sum(torch.exp(my_score),dim = 1)))
  
  dW_t = -torch.mm(X.t(),y_label - soft_max)
  dW = dW_t
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
